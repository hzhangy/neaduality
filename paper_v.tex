...
\subsection{Dimension–Force Mapping and Topological Phase Transition Boundaries}
We define each dimensional transition point as a \textbf{topological phase transition boundary}, marking the hybrid phase between two forces and the jump in dimension.

\begin{table}[H]
\centering
\caption{Dimension–force mapping and phase transition boundaries}
\begin{tabularx}{\textwidth}{c c X X}
\toprule
Ordinal & Force & Geometric operation & Physical manifestation \\
\midrule
$0\to1$ & Weak & 0D point $\to$ 1D line & Pre-geometric singularity, birth of causality \\
$1\to2$ & Electromagnetic & 1D line $\to$ 2D surface & Electroweak unification \\
$2\to3$ & Gravity & 2D surface $\to$ 3D volume & Dark matter effect (see Paper IV) \\
...
\subsection{Zig-zag Evolution and Inflation as Spectral Dimension Transient}
Using a continuous evolution parameter $\chi$ that sequentially activates the four stages, the spectral dimension $d_s$ exhibits a clear stepwise increase (Fig.~\ref{fig:zigzag}). Near each phase transition boundary, $d_s$ temporarily exceeds the theoretical limit (e.g., $\chi\approx0.16$ gives $d_s=3.50$, $\chi\approx1.42$ gives $d_s=4.04$). This ``spectral dimension transient'' corresponds to cosmic inflation: a small-world effect during dimension jumps, eliminating the need for an inflaton field.

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{figures/zigzag_map.png}
\caption{Universal zig-zag map of cosmic history: spectral dimension $d_s$ vs. evolution parameter $\chi$. Transient peaks appear at phase transition boundaries. Data from \texttt{nea\_cosmos\_zigzag\_map.py}.}
\label{fig:zigzag}
\end{figure}

\section{Preliminary Numerical and Observational Evidence}
\subsection{Weak Force Stage ($0\to1$): 1D Causal Chain}
A 1D chain (nodes $i$ connected to $i+1$) yields normalized Laplacian spectral dimensions $d_s$ approaching 1 as $N$ increases ($N=100,200,500$ give $0.9906,0.9730,0.9629$). Code: \texttt{weak\_force\_exp.py}. The $0\to1$ boundary (pre-geometric singularity) remains an open problem; its detailed computational origin is unknown, but this does not affect the stability of the subsequent ladder.

\subsection{Electromagnetic Force Stage ($1\to2$): 2D Weaving}
The ``2D Weaver'' graph is built by adding vertical stitching to a 1D chain (node $i$ connected to $i+\sqrt{N}$). Finite-size scaling from $N=100$ to $10000$ (Table~\ref{tab:em_fss}, Fig.~\ref{fig:em_fss}) shows that $d_s$ increases slowly with system size, reaching $d_s\approx2.11$ at $N=10000$. The convergence limit is not yet determined; we conjecture that this deviation from 2.0 is a finite-size artifact of the stitching density. In the thermodynamic limit ($N\to\infty$) with relative sparsity constraints, $d_s$ may relax to exactly 2.0, but larger-scale simulations are required to confirm.
...
\caption{Finite-size scaling of the 2D Weaver: spectral dimension $d_s$ vs. $1/L$. The data show a slow increase; the convergence limit remains uncertain. The deviation from 2.0 is likely a finite-size artifact.}
\label{fig:em_fss}
\end{figure}
...
Using the SPARC database (138 galaxies), the model $V_{\text{obs}}^2 = V_{\text{bar}}^2[1+(R/R_c)^{2-q}]$ gives a $q$ distribution with a strong peak near $1.0$ and a broad tail extending toward $2.0$, as detailed in Paper IV. A blind test on the extreme low-surface-brightness galaxy UGC~128, using the theoretically derived critical surface density $\Sigma_{\text{crit}}=cH_0/(8\pi G)$, predicts $q_{\text{pred}}=1.017$; the observed fit yields $q_{\text{obs}}=1.001\pm0.210$. The agreement to within $1.6\%$ provides a powerful independent consistency check for the dimensional ladder. This test involves no free parameters and constitutes the strongest observational support for the $2\to3$ transition.

\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{figures/q_distribution.png}
\caption{SPARC $q$ distribution (138 galaxies) showing a strong peak near 1.0 and a broad tail extending toward 2.0. Data from Paper IV.}
\label{fig:qdist}
\end{figure}
...
\]
This is the discrete graph projection of 3D rotational symmetry $SO(3)$. In group-theoretic terms, the cubic symmetry group $O_h$ possesses a three-dimensional irreducible representation $T_{1u}$. Adding $K_4$ cliques (simulating strong anchoring) breaks the degeneracy, splits the eigenvalues, and shifts them downward (Fig.~\ref{fig:triplet_anchored}), mimicking mass generation. This symmetry breaking is topologically equivalent to spontaneous symmetry breaking in Yang–Mills theory. Code: \texttt{str\_force\_exp.py}. The $K_4$ choice is the smallest simplex maintaining 3D rigidity; its symmetry breaking provides a numerical demonstration of the topological origin of fermion generations.

\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/strong_L12_0_0.png}
\caption{Triple degeneracy of eigenvalues in a pure 3D grid, reflecting the $T_{1u}$ representation of $O_h$.}
\label{fig:triplet_pure}
\end{figure}
\begin{figure}[H]
\centering
\includegraphics[width=0.45\textwidth]{figures/strong_L12_400_4.png}
\caption{Splitting of eigenvalues after adding $K_4$ anchors, analogous to spontaneous symmetry breaking.}
...
\begin{itemize}
    \item The $0\to1$ boundary (pre-geometric singularity) remains an open problem whose computational origin is unknown.
...
\section{Conclusion}
We have presented a heuristic unified perspective, ``topological genesis of dimensions,'' supported by preliminary numerical experiments and astronomical observations. The 1D chain spectral dimension, finite-size scaling of 2D weaving, galaxy rotation curve blind test (1.6\% precision, referencing Paper IV), triple degeneracy in 3D grids interpreted as $O_h$ group representation, and black hole dimensional collapse are all consistent with the $0$-$1$-$2$-$3$-$4$ zig-zag ladder. Despite open issues and the pre-geometric singularity at $0\to1$, this work provides a new testable framework for understanding fundamental forces as anchors of dimensionality. The validity of the ladder awaits stricter mathematical proofs and larger-scale simulations.
...
The author thanks the AI collaboration team for intensive computational support.
...
\bibitem{book} Zhang, Y. (2025). \textit{Being is Time}. ISBN 978-1-917850-74-2 (Chinese Version). ISBN 978-1-80609-401-1 (English Version).
...
\bibitem{paperIV} Zhang, Y. (2026). Dark Matter as a Projection Artifact: Observational Evidence from SPARC Galaxy Rotation Curves. Zenodo. https://doi.org/10.5281/zenodo.19483095
\end{thebibliography}
